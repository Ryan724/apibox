...
*根据接口描述，执行操作
*/
app.post("/api/project/add",function(request){
	var pro ={"name":"zpj","desc":"帅"};
	if(Project.add(pro)) return result(true,"项目添加成功")
	return result(false,"项目添加失败")
});
app.post("/api/interface/add",function(request){
	var api = {"config": {"name": "这是接口名", "desc": "这是接口的详细描述", "project":"9317f0a3ba10414d6e3ac53badd59906","url": "http://example.com/demo", "method": ["GET"]}, "request": {"id": "1000"}, "response": {"success": true, "model": {"title": "string", "list": [{"id": 1000, "name": "name-123"}, {"id": 1000, "name": "name-123"} ] } } };
	if(Interface.add(api))return result(true,"接口添加成功")
	return result(false,"接口添加失败")
});
app.post("/api/interface/update",function(request){
