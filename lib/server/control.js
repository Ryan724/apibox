...
var Mock = require('../mockserver/service');
...
*项目请求
*/
app.post("/api/project/add",function(request,response,data){
 ...
 * api的请求
 */
app.post("/api/interface/getmockurl", function(request,response,data) {
	data = JSON.parse(data[0]);
	if (!Interface.writeMockData(data.pid,data.id,data.mock)) return result(false, "接口mock属性添加失败");
	var mockUrl = Mock.generateUrl({pid:data.pid,mid:data.id});
...
app.post("/api/mock/getserverdate", function(request,response,data) {
	...
	httpClient.getServerData(data.pid,data.id,function(serverData){
		serverData.data = serverData.data?serverData.data[0]:"";
