...
var DIR = parentDir(parentDir(__dirname)) + (config.env=="dev"?"/app/":"/release/app/");
var app = {
	start: function() {
		http.createServer(serverCallback).listen(config.server.port, config.server.host);
		...
		try {
			...
			if (exists) {
				...
				if (request.url.indexOf("api") > 0) {
					control(request,response, function(resultStr) {
